import React from 'react';
import { Dialog, DialogContent, DialogActions, Button, Box, Typography, TextField } from '@mui/material';
import { useTranslation } from 'react-i18next';
import { theme } from '../../theme/theme';

interface CampaignCategoryNameDialogProps {
  open: boolean;
  editingCategoryId: string | null;
  categoryNameInput: string;
  onCategoryNameChange: (value: string) => void;
  onClose: () => void;
  onSave: () => void;
  onOpenItemsDialog: () => void;
}

const CampaignCategoryNameDialog: React.FC<CampaignCategoryNameDialogProps> = ({
  open,
  editingCategoryId,
  categoryNameInput,
  onCategoryNameChange,
  onClose,
  onSave,
  onOpenItemsDialog,
}) => {
  const { t } = useTranslation();

  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      <DialogContent sx={{ p: theme.spacing.lg }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.md }}>
          <Typography
            variant="h6"
            fontWeight={theme.typography.fontWeights.semibold}
            sx={{ color: theme.colors.text }}
          >
            {editingCategoryId === null
              ? t('admin.campaignEditor.dialog.addCategory')
              : t('admin.campaignEditor.dialog.renameCategory')}
          </Typography>
          <TextField
            autoFocus
            fullWidth
            label={t('admin.campaignEditor.dialog.categoryName')}
            value={categoryNameInput}
            onChange={(event) => onCategoryNameChange(event.target.value)}
          />
        </Box>
      </DialogContent>
      <DialogActions sx={{ px: theme.spacing.lg, pb: theme.spacing.lg }}>
        <Button
          variant="outlined"
          onClick={onOpenItemsDialog}
          sx={{ textTransform: 'none', mr: 'auto' }}
          disabled={editingCategoryId === null}
        >
          {t('admin.campaignEditor.dialog.addRemoveItemsFromMenu')}
        </Button>
        <Button onClick={onClose} sx={{ textTransform: 'none' }}>
          {t('common.cancel')}
        </Button>
        <Button variant="contained" onClick={onSave} sx={{ textTransform: 'none' }}>
          {t('common.save')}
        </Button>
      </DialogActions>
    </Dialog>
  );
};

export default CampaignCategoryNameDialog;
